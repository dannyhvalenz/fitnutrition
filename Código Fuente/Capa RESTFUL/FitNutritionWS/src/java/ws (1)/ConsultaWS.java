...
public class ConsultaWS {
    ...
    @Path("getConsultaByIDPaciente")
    @POST
    @Produces(MediaType.APPLICATION_JSON)
    public Consulta getConsultaByIDPaciente(@FormParam("idPaciente") Integer idPaciente){
        SqlSession conn = MyBatisUtil.getSession();
        Consulta consulta =  new Consulta();
        if(conn != null){
            try {
                consulta = conn.selectOne("Consulta.getConsultaByIDPaciente", idPaciente);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    @Path("getConsultasOfPaciente")
    @POST
    @Produces(MediaType.APPLICATION_JSON)
    public List<Consulta> getConsultasOfPaciente(@FormParam("idPaciente") Integer idPaciente){
        List<Consulta> consultas = null;
        SqlSession conn = MyBatisUtil.getSession();
        Consulta consulta =  new Consulta();
        if(conn != null){
            try {
                consultas = conn.selectList("Consulta.getConsultasOfPaciente", idPaciente);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
